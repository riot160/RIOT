...
// ✅ FIX 1: Use shared sessions store to avoid circular require
...
// ✅ FIX 2: sock.end() is not a valid Baileys method — use ws?.close() instead
...
// ✅ FIX 3: Improved phone number formatting — no longer forces 254 on all numbers
function formatPhoneNumber(input) {
    let clean = input.toString().replace(/\D/g, '');

    // Handle local Kenyan numbers starting with 0
    if (clean.startsWith('0') && clean.length === 10) {
        clean = '254' + clean.substring(1);
    }

    // If no country code present (less than 11 digits), assume Kenya
    if (clean.length < 11) {
        clean = '254' + clean;
    }
...
app.get('/dashboard', auth.requireAuth, async (req, res) => {
    ...
    // ✅ FIX 4: sessions now imported directly — no more require('./index')
...
app.post('/api/pair', async (req, res) => {
    ...
    try {
        ...
        sock = makeWASocket({
            version,
            auth: state,
            browser: Browsers.macOS('ALMEER MD'),
        ...
        // ✅ FIX 5: Properly wait for connection before requesting pairing code
        ...
        // ✅ FIX 6: Cleanup after code is sent — delay gives WhatsApp time to process
        setTimeout(async () => {
            closeSock(sock);
...
app.get('/api/simple-pair', async (req, res) => {
    ...
    try {
        ...
        sock = makeWASocket({
            auth: state,
            browser: Browsers.ubuntu('ALMEER MD'),
            logger: pino({ level: 'silent' }),
